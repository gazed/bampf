...
func newHud(eng *vu.Eng, sentryCount int) *hud {
	hd := &hud{}
...
func (hd *hud) setSize(screenX, screenY, screenWidth, screenHeight int) {
	...
	hd.scene.SetOrthographic(0, float32(hd.w), 0, float32(hd.h), 0, 10)
...
func (hd *hud) resize(screenWidth, screenHeight int) {
	...
	hd.ce.SetScale(float32(hd.w), float32(hd.h), 1)
	hd.ce.SetLocation(hd.cx, hd.cy, -1)
	hd.te.SetScale(float32(hd.w), float32(hd.h), 1)
	hd.te.SetLocation(hd.cx, hd.cy, -1)
	hd.ee.SetScale(float32(hd.w), float32(hd.h), 1)
	hd.ee.SetLocation(hd.cx, hd.cy, -1)
}

// setVisible turns off the HUD.  This is used when transitioning between
// levels.
...
// outside interface.
func (hd *hud) addWall(gamex, gamez float32)             { hd.mm.addWall(gamex, gamez) }
func (hd *hud) remCore(gamex, gamez float32)             { hd.mm.remCore(gamex, gamez) }
func (hd *hud) addCore(gamex, gamez float32)             { hd.mm.addCore(gamex, gamez) }
...
// an alternative view (albeit, less usefull) of the current players health.
// The player view is updated due to the trooper model being updated as it
// gains or loses health.
//
// Player can ignore resizes since it is in the lower left corner.
type player struct {
	cx, cy float32  // Center location.
...
func newPlayer(eng *vu.Eng, scene vu.Scene, screenWidth, screenHeight int) *player {
	pl := &player{}
	pl.cx, pl.cy = 100, 100
	pl.bg = scene.AddPart()
	pl.bg.SetFacade("icon", "uvm", "green")
...
// setLevel is called when there is a level change. This method alters the player
// direction and location to show it at an angle (since nothing else is
// using the player rotation/location fields).
...
// progress bars to allow for different progress display effects.
type xpbar struct {
	area
	eng    *vu.Eng
...
func newXpbar(e *vu.Eng, scene vu.Scene, screenWidth, screenHeight int) *xpbar {
	xp := &xpbar{}
	xp.eng = e
	xp.border = 5
	xp.linew = 2
	...
	xp.bg = scene.AddPart()
	xp.bg.SetFacade("square", "flat", "tblack")
	xp.fg = scene.AddPart()
	xp.fg.SetFacade("icon", "uv", "green")
	...
	xp.hb.SetLocation(xp.cx-float32(bw/2), xp.cy*0.5-5, 0)

	// teleport energy background and foreground bars.
	xp.tbg = scene.AddPart()
	xp.tbg.SetFacade("square", "flat", "tblack")
	xp.tfg = scene.AddPart()
	xp.tfg.SetFacade("icon", "uv", "green")
	...
	xp.cbg.SetFacade("square", "flat", "tblack")
	xp.cfg = scene.AddPart()
	xp.cfg.SetFacade("icon", "uv", "green")
...
func (xp *xpbar) resize(screenWidth, screenHeight int) {
	...
	xp.bg.SetScale(float32(xp.bw/2), float32(xp.bh-xp.y), 1)

	// adjust the teleport energy bar.
	xp.tbg.SetLocation(xp.cx-float32(xp.w)/10-2, xp.cy+30, 1)
	xp.tbg.SetScale(float32(xp.bw/10), float32(xp.bh-xp.y)-5, 1)
	bw := xp.tk.BannerWidth()
	xp.tk.SetLocation(xp.cx-float32(xp.bw)/10-2-float32(bw/2), xp.cy+21, 0)

	// adjust the cloaking energy bar.
	xp.cbg.SetLocation(xp.cx+float32(xp.bw)/10+2, xp.cy+30, 1)
	xp.cbg.SetScale(float32(xp.bw/10), float32(xp.bh-xp.y)-5, 1)
	bw = xp.ck.BannerWidth()
	xp.ck.SetLocation(xp.cx+float32(xp.bw)/10+2-float32(bw/2), xp.cy+21, 0)
...
func (xp *xpbar) setSize(screenWidth, screenHeight int) {
	...
	xp.cx, xp.cy = float32(screenWidth)*0.5-float32(xp.border), float32(xp.bh)*0.5+float32(xp.border)
}

// healthMonitor.healthUpdated. Only update the banner when it changes.
func (xp *xpbar) healthUpdated(health, warn, high int) {
	...
	barMax := float32(xp.bw/2 - xp.linew)
	if health >= warn {
		xp.fg.SetTexture("xpcyan", 0)
	} else {
		xp.fg.SetTexture("xpred", 0)
	}
	healthBar := float32(health) / float32(high) * barMax
	zeroSpot := float32(xp.border) + healthBar + float32(xp.linew-xp.border)
	xp.fg.SetLocation(zeroSpot, xp.cy, 0)
	xp.fg.SetScale(healthBar, float32(xp.bh-xp.y-xp.linew)-1, 1)
}

// energyMonitor.energyUpdated. Only update the energy when it changes.
func (xp *xpbar) energyUpdated(teleportEnergy, tmax, cloakEnergy, cmax int) {
	tratio := float32(teleportEnergy) / float32(tmax)
	if tratio == 1.0 {
		xp.tfg.SetTexture("xpblue", 0)
	} else {
		xp.tfg.SetTexture("xpred", 0)
	}
	xp.tfg.SetLocation(xp.cx-float32(xp.w)/10-2, xp.cy+30, 0)
	xp.tfg.SetScale((float32(xp.bw/10)-2)*tratio, float32(xp.bh-xp.y)-7, 1)
	cratio := float32(cloakEnergy) / float32(cmax)
	xp.cfg.SetLocation(xp.cx+float32(xp.w)/10+2, xp.cy+30, 0)
	xp.cfg.SetScale((float32(xp.bw/10)-2)*cratio, float32(xp.bh-xp.y)-7, 1)
}
...
// newMinimap initializes the minimap. The setLevel and add* methods still
// need to be called to populate it.
func newMinimap(eng *vu.Eng, numTroops int) *minimap {
	...
	mm.scene.SetVisibleRadius(float32(mm.radius))
	mm.scene.Set2D()
	mm.setSize(eng.Size())
	...
	mm.part.SetLocation(float32(mm.x), 0, float32(-mm.y))

	// add the white background.
	mm.bg = mm.part.AddPart()
	mm.bg.SetFacade("icon_xz", "uvm", "green")
	...
	for cnt := 0; cnt < numTroops; cnt++ {
		tpm := mm.part.AddPart()
		tpm.SetFacade("square_xz", "flat", "tred")
	...
	mm.cpm.SetFacade("square_xz", "flat", "blue")
	mm.cpm.SetScale(mm.scale, mm.scale, mm.scale)
	mm.ppm = mm.part.AddPart()
	mm.ppm.SetFacade("tri_xz", "flat", "tblack")
...
func (mm *minimap) setSize(x, y, width, height int) {
	...
	mm.scene.SetOrthographic(0, float32(mm.w), 0, float32(mm.h), 0, 10)
}
...
func (mm *minimap) setLevel(sc vu.Scene, lvl *level) {
	...
	mm.cx, mm.cy = float32(lvl.gcx*lvl.units)*mm.scale, float32(-lvl.gcy*lvl.units)*mm.scale
...
func (mm *minimap) addWall(x, y float32) {
	wall := mm.part.AddPart()
	wall.SetFacade("square_xz", "flat", "gray")
...
func (mm *minimap) addCore(x, y float32) {
	scale := mm.scale
	cm := mm.part.AddPart()
	cm.SetFacade("square_xz", "flat", "green")
...
func (mm *minimap) remCore(gamex, gamez float32) {
	scale := mm.scale
	gx, gz := gamex*scale, gamez*scale
...
func (mm *minimap) setPlayerLocation(x, y, z float32) {
	mm.ppm.SetLocation(x, y, z)
	mm.bg.SetLocation(x, y, z)
}
func (mm *minimap) setPlayerRotation(x, y, z, w float32) { mm.ppm.SetRotation(x, y, z, w) }

// set the position of the maze center marker.
func (mm *minimap) setCenterLocation(x, y, z float32) {
	...
	if dtoc > float32(mm.radius) {          // ... unless the distance is to great
		toc.Unit().Scale(float32(mm.radius))
